  ...
  package Phases "Phases or mixtures of species"
    ...
    equation
      ...
      connect(H2.chemical, chemical.H2) annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect(H2.joint.mechanical, joint.mechanical) annotation (Line(
      ...
      connect(H2O.chemical, chemical.H2O) annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect(H2O.joint.mechanical, joint.mechanical) annotation (Line(
          points={{6.10623e-16,6.10623e-16},{5.55112e-16,5.55112e-16}},
          color={72,90,180},
          smooth=Smooth.None));
      ...
      connect(N2.chemical, chemical.N2) annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect(N2.joint.mechanical, joint.mechanical) annotation (Line(
          points={{6.10623e-16,6.10623e-16},{5.55112e-16,5.55112e-16}},
          color={72,90,180},
          smooth=Smooth.None));
      ...
      connect(O2.chemical, chemical.O2) annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect(O2.joint.mechanical, joint.mechanical) annotation (Line(
          points={{6.10623e-16,6.10623e-16},{5.55112e-16,5.55112e-16}},
          color={72,90,180},
          smooth=Smooth.None));
 ...
 <a href=\"modelica://FCSys.Subregions.Phases.BaseClasses.NullPhase\">NullPhase</a> model.</p></html>"),
    ...
    equation
      ...
      connect(C.chemical, chemical.C) annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect(C.joint.mechanical, joint.mechanical) annotation (Line(
          points={{6.10623e-16,6.10623e-16},{5.55112e-16,5.55112e-16}},
          color={72,90,180},
          smooth=Smooth.None));
      ...
      connect('e-'.chemical, chemical.'e-') annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect('e-'.joint.mechanical, joint.mechanical) annotation (Line(
          points={{6.10623e-16,6.10623e-16},{5.55112e-16,5.55112e-16}},
          color={72,90,180},
          smooth=Smooth.None));
 ...
 <a href=\"modelica://FCSys.Subregions.Phases.BaseClasses.NullPhase\">NullPhase</a> model.</p></html>"),
    ...
    model Ionomer "Ionomer phase"
      extends BaseClasses.NullPhase(final n_spec=countTrue({inclC19HF37O5S,
            inclH2O,'inclH+'}), joint(mechanical(phi(fixed=if inclC19HF37O5S
                   then fill(false, n_vel) else {initVelX,initVelY,initVelZ}[
                  cartAxes]))));
    ...
    equation
      ...
      connect(C19HF37O5S.chemical, chemical.C19HF37O5S) annotation (Line(
          ...
          color={208,104,0},
          smooth=Smooth.None));
      connect(C19HF37O5S.joint.mechanical, joint.mechanical) annotation (Line(
          points={{6.10623e-16,6.10623e-16},{5.55112e-16,5.55112e-16}},
          color={72,90,180},
          smooth=Smooth.None));
  ...
  <p>For more information, see the <a href=\"modelica://FCSys.Subregions.Species.Species\">Species</a> model.</p></html>"));
    ...
    model Species
      ...
      output Q.CapacityThermalSpecific c_V(stateSelect=StateSelect.never) =
        Data.c_V(T, p) if defaults.analysis "Isochoric specific heat capacity";
      ...
      output Q.Force mphiI[n_vel, Orientation](each stateSelect=StateSelect.never)
        ...
        velAxes[cartWrap(cartAxes[axis] + orientation)]]*I[axis] else 0) for
      ...
      output Q.Power TsI[n_vel](each stateSelect=StateSelect.never) = T*s*I if
      ...
      output Q.Force f_exch_diff[n_vel](each stateSelect=StateSelect.never) =
        joint.mechanical.mPhidot + inert.mPhidot if defaults.analysis
      ...
      output Q.Force f_trans_adv[n_vel](each stateSelect=StateSelect.never) = {
        ...
        *Delta(APdot_face[cartWrap(cartAxes[axis] - orientation), :,
        orientation] .* Ndot_face[cartWrap(cartAxes[axis] - orientation), :])/A[
        cartWrap(cartAxes[axis] - orientation)] for orientation in Orientation)
        for axis in 1:n_vel}
      ...
      output Q.Force f_trans_diff[n_vel](each stateSelect=StateSelect.never) =
        {Delta(p_face[cartAxes[axis], :])*A[cartAxes[axis]] + sum(Delta(
        tau_face[cartWrap(cartAxes[axis] - orientation), :, orientation])*A[
        cartWrap(cartAxes[axis] - orientation)] for orientation in Orientation)
        for axis in 1:n_vel}
      ...
      output Q.Power Wdot_trans(stateSelect=StateSelect.never) = -sum(sum((
        ...
        Data.v_Tp(T_face[axis, side], p_face[axis, side])*Ndot_face[axis, side])
        ^2 + APdot_face[axis, side, :]*APdot_face[axis, side, :]/A[axis]^2)/2
         - h - Data.m*phi*phi/2)*Ndot_face[axis, side] for side in Side) for
        axis in Axis) if defaults.analysis
      ...
      FCSys.Connectors.Inert joint(
        ...
        heat(T(final start=T_IC,final fixed=false)))
    ...
    protected
      ...
      final parameter Integer cartAxes[n_vel]=index(inclVel)
        "Cartesian-axis indices of the axes of linear momentum";
      final parameter Integer velAxes[Axis]=enumerate(inclVel)
        "Velocity component indices of the Cartesian axes";
      ...
      Q.VolumeRate APdot_face[Axis, Side, Orientation](start={{{if inclVel[
            cartWrap(axis + orientation)] then inSign(side)*A[axis]*phi_IC[
            cartWrap(axis + orientation)] else 0 for orientation in Orientation}
            for side in Side} for axis in Axis})
    ...
    equation
      ...
      for axis in 1:n_vel loop
        if setVel[cartAxes[axis]] then
          ...
          der(M*phi[axis])/U.s = chemical.mPhidot[axis] + joint.mechanical.mPhidot[
